...
def generate_mcq():
    ...
    print(f"Received request for course: {course}, level: {level}")  # Debugging

    if course in questions_data and level in questions_data[course]:
        print(f"Returning questions for {course} - {level}")  # Debugging
        return jsonify({"questions": questions_data[course][level]})
    else:
        print(f"Invalid course or level: {course}, {level}")  # Debugging
